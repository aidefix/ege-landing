...
const W = 900;
const H = 360;
const PAD = { left: 86, right: 86, top: 40, bottom: 28 };

/** Шкала подписана от 40 до 100, поэтому 60 баллов растянуты на всю высоту. */
...
export function Results() {
  return (
    <Sheet id={SECTIONS.res.id} tilt="left" tape="tl">
      ...
      <div
        style={GRID}
        className="rounded-card border-[2.5px] border-ink bg-white p-3 shadow-hard-lg"
      >
        <svg
          viewBox={`0 0 ${W} ${H}`}
          role="img"
          aria-label="Изменение баллов учеников: входной балл и результат ЕГЭ"
          className="block h-auto w-full font-hand text-[15px] font-bold"
        >
